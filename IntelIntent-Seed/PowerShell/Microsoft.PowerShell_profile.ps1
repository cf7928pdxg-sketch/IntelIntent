...
function urmto {
    ...
    $urmtoRoot = $env:URMTO_ROOT
    if (-not $urmtoRoot) {
        $urmtoRoot = "C:\Users\Nic\OneDrive\SolutionPartnerDesignations"
    }
    
    switch ($Command) {
        "status" {
            & "$urmtoRoot\scripts\Get-IntegrationStatus.ps1" @Arguments
        }
        "run" {
            $modulePath = "$urmtoRoot\modules\URMTO-RecursiveOpt.psm1"
            if (Test-Path $modulePath) {
                Import-Module $modulePath -Force
                Start-RecursiveOptimization @Arguments
            } else {
                Write-Error "URMTO-RecursiveOpt module not found at $modulePath"
            }
        }
        "dashboard" {
            & "$urmtoRoot\scripts\Get-IntegrationStatus.ps1" -ExportToHtml @Arguments
        }
        "help" {
            Write-Host "URMTO Commands:" -ForegroundColor Cyan
            Write-Host "  urmto status   - Get current integration status"
            Write-Host "  urmto run      - Run recursive optimization"
            Write-Host "  urmto dashboard - Generate and open HTML dashboard"
            Write-Host "  urmto help     - Show this help message"
        }
        default {
            Write-Error "Unknown command: $Command. Use 'urmto help' for available commands."
        }
    }
}

# Set alias for backward compatibility
Set-Alias -Name optimize -Value urmto
